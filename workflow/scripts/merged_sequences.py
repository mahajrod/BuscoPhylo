...
def main():
    ...
    for dir in args.input:
        for id in Path(dir).glob("*.fna"):
            ids_dict[id.stem] += 1
    common_ids = set()
    number_of_species = len(args.input)
    for key, value in ids_dict.items():
        if value == number_of_species:
            common_ids.add(key)
    item_count = 0
    group_number = 0
    for id in common_ids:
        if item_count % args.group_size == 0 or item_count == 0:
            group_number_dir = outdir / str(group_number)
            group_number_dir.mkdir()
            group_number += 1
        file_faa = id + ".faa"
        file_fna = id + ".fna"
        out_faa = open(group_number_dir / file_faa, 'a')
        out_fna = open(group_number_dir / file_fna, 'a')
        for dir in args.input:
            dirpath = Path(dir)
...
if __name__ == "__main__":
    ...
    group_required.add_argument('-i', '--input', type=str, nargs='+', help="single_copy_busco_sequences directories")
    group_required.add_argument('-o', '--outdir', type=str, help="output directory name")
    group_required.add_argument('-g', '--group-size', type=int, help="group size")
