...
checkpoint mafft:
    input:
        mafft_task=expand(mafft_dir_path / 'slurm/mafft.tasks.{i}.sh', i=glob_wildcards(os.path.join(checkpoints.mafft_tasks_list.get(**wildcards).output[0], 'mafft.tasks.{i}.sh')).i)
    output:
